...
|---|---|---|
| FIC-01 | Every date in the seed is an offset from the moment the app loads, so the world is always mid-flight. [[SLICE-05]] built ENG-09, the one clock service, server side, and re-pointed the four wired surfaces that read the server (the notifications bell, `/notifications`, the live obligation detail's ladder rows, the evidence drawer's captured-at) at a real client clock, `apps/web/src/lib/clock.ts`. The remaining 90-odd seed files still import `apps/web/src/lib/time.ts`'s `NOW`, a page-load anchor, and retire module by module (D-030, D-031) | Nothing in production may depend on an anchor. Every ledger, every age, every countdown and every period comparison in the prototype is staged, and none of it is a requirement |
| FIC-02 | The marquee incident's six-hour clock reads 03:11:42 remaining on first paint, on any date. Untouched by [[SLICE-05]]: the incident module has no table, the countdown mechanism itself is correct by design (CLK-012, a browser countdown ticks from a fixed deadline and does not poll) and stays fiction until the incident module supplies a real deadline | A real incident is usually quiet, sometimes breached, and occasionally three minutes old. Screens tuned to one dramatic value look wrong at every other value |
...
| ID | The fiction | What breaks |
|---|---|---|
| FIC-16 | No table is ever empty, anywhere. This was a deliberate seeding aesthetic. [[SLICE-05]] built the four messages plus the scoped-away case, at `apps/web/src/components/states/`, and applied them to the surfaces that already read the server. The 26 screens that still read the seed keep this fiction until each adopts the vocabulary in its own module's slice (STATE-080, D-030, D-031) | Day one is empty. A filter that matches nothing is empty. A module not yet deployed is empty. A quiet period is empty. Those are four different messages and the prototype has none of them |
