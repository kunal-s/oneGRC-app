...
|---|---|---|---|---|---|---|---|---|---|---|
| INS-T1 | n/a | Draft or In force | A person supplies a name and URL, or uploads a document | Compliance Manager in Compliance and Company Secretarial, or the Administrator. D-037 | The document is retrievable or uploaded, the entry method is recorded, and the departments the instrument routes to are assigned. D-032, D-033 | Instrument created, carrying its assigned `departments` | Document stored by content hash; extraction starts; the assigned departments gain visibility | `instrument.create` naming actor, entry method, source and the assigned departments | `POST /instruments` | LIVE via `pnpm ingest`, without the department assignment |
...
| INS-I3 | Creating an instrument from outside Compliance and Company Secretarial, other than by the Administrator | Deciding that a body of law binds the firm is that function's accountability. `BR-AUT-02`, D-037 |
| INS-I4 | Removing a department from an instrument's list by derivation | The widening in INS-T1 and CLS-T3 is additive only. An act must not vanish from a department's library because the last control citing it was retired. Removing one is a deliberate act by the same authority that assigned it. D-032 |
| INS-I5 | Creating an instrument with no document and no recorded reason for having none | Every clause traces to the page it came from. The one lawful case is a licensed standard the firm may not copy, which is registered with text layer `none` and its clauses keyed by hand. D-033 |
