...
| CYC-T4 | In review | Due | The checker returns a task with a reason | Compliance Manager, Executive, Auditor, and never the submitter | A reason is supplied | `state`; the task returns to InProgress | The maker gains a queue item carrying the reason; the ladder resumes | `task.return` with the reason | `POST /tasks/:id/return` | SIMULATED, the transition itself: `POST /tasks/:id/return` does not exist yet. The ladder-resumes side effect needs no separate wiring once it does, for the reason CYC-T1 gives: the registry reads current state, so a cycle back at Due is chased again the next tick with nothing told to it |
| CYC-T5 | Due | Due | An exception is approved against the duty | Compliance Manager, Risk Manager, Executive | The exception is Active | none on the cycle | The ordinary ladder stops running against this cycle and the exception's expiry ladder starts | `exception.approve` | `POST /exceptions/:id/approve` | SIMULATED. Still true after [[SLICE-02]]: E-46 Exception has no table, so nothing can read Active against it, and the ordinary ladder has no stop condition to check. [[SLICE-24]] |
...
| ID | From | To | Trigger | Who may | Must be true first | State change | Side effects | Audit | API write | In prototype |
|---|---|---|---|---|---|---|---|---|---|---|
| TSK-T1 | n/a | Open | The parent record creates it | System | A parent cycle, campaign, issue or request exists | Task created with a policy, an assignee, a nominated checker and a due date | The assignee gains a queue item; the ladder starts against the task's own due date | `task.create` | internal | LIVE. The ladder-starts side effect is real, [[SLICE-02]], only for a task that is one of several on its cycle (TSK-I6): a cycle's one ordinary task is already fully chased by the cycle's own ladder (CYC-T1), so it carries no second, identical one of its own |
...
| TSK-T6 | Submitted | Returned | The checker returns with a reason | Compliance Manager, Executive, Auditor, and never the maker | A reason is supplied | `state`, `returnReason` | The maker gains a queue item carrying the reason; the ladder resumes | `task.return` with the reason | `POST /tasks/:id/return` | SIMULATED, the transition itself: `POST /tasks/:id/return` does not exist yet. As with CYC-T4, the ladder needs no separate resume once it does |
...
| TSK-I6 | Chasing a multi-step duty at the duty level only | A three-party payroll chain has three accountable people, and chasing only the first is chasing nobody. `BR-ESC-06`. Enforced, [[SLICE-02]]: a task registers its own ladder only where its cycle carries more than one task. No screen today can create a multi-task cycle (`POST /controls/:id/obligations` always creates exactly one), so this line is built and unit-testable but not yet click-verified against a real multi-step duty; [[SLICE-16]] is where one first can be |
