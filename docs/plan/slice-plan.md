...
| 03 | [[SLICE-03\|Files, in and out]] | SCR-100, SCR-101, SCR-102 | E-13, E-31 | M-01, M-05 | 01A | Real upload with scanning and limits, content-addressed storage with retention, and real export under the caller's own scope | **in progress**, work order at `docs/slices/SLICE-03-files-in-and-out.md`. Scoped against what exists today: the upload path is the one governed write that creates evidence, `POST /tasks/:id/evidence`, reached from the live obligation detail; the export path covers the two registers read from the server, `GET /controls` whose boundary [[SLICE-01C]] made real, and `GET /instruments` which has none yet and says so on the face of the export. Every other register gains its export inside its own module's slice. SCR-100 and SCR-101 are built as the drawers the inventory names: both kinds are declared in the store today and `DrawerHost.tsx` has no case for either, so both render the generic panel. SCR-102 is shared with [[SLICE-46]], which keeps the PDF report templates. Adds `export.run` to the authority table, which `platform.md` section 4 does not carry while section 6 already carries REF-29 for an export beyond scope. Raises DN-038 to DN-042, and ER-015 and ER-016, none blocking |
